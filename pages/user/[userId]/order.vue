...
// 取得訂單資訊
const { data:ordersList, error, refresh } = await useAsyncData('order-detail', async()=> {
...
const historyOrderList = computed(() =>
  ordersList.value?.filter(order => order.status === -1).slice(0, historyOrderListDisdayNum.value) || []
);
// 展示更多歷史訂單
...
<template>
  <div class="row gap-6 gap-md-0">
    <div class="col-12 col-md-7">
      <div
        class="rounded-3xl d-flex flex-column gap-6 gap-md-10 p-4 p-md-10 bg-neutral-0 mb-md-5"
      ...
      >
        ...
        <section class="d-flex flex-column gap-6">
          <h3 class="d-flex align-items-center mb-0 text-neutral-80 fs-8 fs-md-6 fw-bold">
            <p class="mb-0">
              {{ order.roomId.name }}，{{ totalDate(order.checkOutDate, order.checkInDate) }} 晚
            </p>
            <span
          ...
          <p class="mb-0 text-neutral-80 fs-8 fs-md-7 fw-bold">
            NT$ {{ formatToThousand(order.roomId.price * totalDate(order.checkOutDate, order.checkInDate)) }}
          </p>
        </section>
          ...
          <div class="d-flex flex-column flex-lg-row gap-6 mb-6 mb-md-10">
            ...
            <section class="d-flex flex-column gap-4">
              ...
              <div class="text-neutral-80 fw-medium">
                <p class="mb-2">
                  住宿天數： {{ totalDate(historyOrder.checkOutDate, historyOrder.checkInDate) }} 晚
                </p>
                <p class="mb-0">
                  住宿人數：{{ historyOrder.pepleNum  }} 位
                </p>
              </div>
              ...
              <div class="text-neutral-80 fs-8 fs-md-7 fw-medium">
                ...
                <p
                  class="title-deco mb-0"
                >
                  退房：{{ formatDate(historyOrder.checkOutDate) }}，12:00 前退房
                </p>
              </div>
              <p class="mb-0 text-neutral-80 fs-8 fs-md-7 fw-bold">
                NT$ {{ formatToThousand( historyOrder.roomId.price * totalDate(historyOrder.checkOutDate, historyOrder.checkInDate) ) }}
              </p>
            </section>
          </div>
  
          <hr class="my-0 opacity-100 text-neutral-40" v-if="index + 1 !== historyOrderList.length">
        </div>
